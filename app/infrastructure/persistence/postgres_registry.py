"""Реестр сессий и пользователей в PostgreSQL."""
...
class PostgresRegistry:
    ...
    def save_user(self, user: dict[str, Any]) -> str:
        """
        Создаёт пользователя или обновляет по username.
        Нужно при миграции из Redis: там другой UUID, чем у сида admin в Postgres —
        без этого получается UniqueViolation на username.
        """
    ...
    def upsert_session_from_migrate(self, document: dict[str, Any]) -> None:
        """Используется скриптом миграции из Redis (полная замена строки)."""
        sid = document.get("id")
