...
| Code | Adjusted `src/app/core/config.py` project root resolution — the path was previously hardcoded relative to `app/` and broke once the folder was nested under `src/`; fixed `sys.path` in scripts for the same reason |
