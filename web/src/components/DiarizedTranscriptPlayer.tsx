...
/**
 * Seek + play. ВАЖНО: не используем Math.min(sec, dur) для всех случаев — если метаданные
 * отдают кривой короткий duration, иначе любой переход к 45 с превращается в ~1 с.
 * Обрезаем только если явно выходим за конец трека.
 */
export function seekAudioToTime(
  ...
  const apply = () => {
    ...
    // Сразу вызывать play() после currentTime нельзя: в части браузеров старт идёт с прежней позиции (часто 0).
...
/**
 * Границы сегмента в секундах. Бэкенд отдаёт start_sec/end_sec, но в JSON могут быть
 * start_ms/end_ms (сырой ASR) или camelCase после других слоёв.
 */
...
/** В payload есть словоуровневые сегменты — для UI лучше поток текста, не столбец кнопок. */
...
type DiarizedTranscriptPlayerProps = {
  ...
  /** Длительность из payload (сек) — если браузер отдаёт кривой duration, не душим seek */
  hintDurationSec?: number | null
  /** true: токены в одном абзаце (клик по слову). false: строки по сегментам спикера */
  wordStream?: boolean
}
...
export function DiarizedTranscriptPlayer({
  ...
  /** Текущая позиция воспроизведения — для караоке (прошлые слова приглушены). */
