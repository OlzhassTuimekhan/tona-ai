...
def get_org_ratings(
    ...
    for row in rows:
        org_name = row.get("public_org") or "(no organization)"
...
def _save_photo(upload: UploadFile) -> str:
    ...
    if ext not in _ALLOWED_PHOTO_EXT:
        raise HTTPException(
            status_code=400,
            detail=f"Unsupported format {ext}. Allowed: {', '.join(_ALLOWED_PHOTO_EXT)}",
        )
    ...
    if dest_path.stat().st_size < 100:
        dest_path.unlink(missing_ok=True)
        raise HTTPException(status_code=400, detail="photo_invalid_or_empty")
...
async def add_public_observation(
    ...
    if website.strip():
        raise HTTPException(status_code=400, detail="request_rejected")

    if observation_type not in ("was_there", "work_done", "dispute"):
        raise HTTPException(
            status_code=400,
            detail="invalid_observation_type",
        )
    ...
    if commitment_index is not None and str(commitment_index).strip() != "":
        try:
            ...
            raise HTTPException(
                status_code=400,
                detail="invalid_commitment_index",
            ) from e
        if idx < 0:
            raise HTTPException(
                status_code=400,
                detail="commitment_index_negative",
            )
    ...
    try:
        ...
        if tmp_path.stat().st_size > _MAX_VOICE_BYTES:
            raise HTTPException(
                status_code=400,
                detail="voice_file_too_large",
            )
        if tmp_path.stat().st_size < 256:
            raise HTTPException(
                status_code=400,
                detail="voice_file_too_short",
            )
