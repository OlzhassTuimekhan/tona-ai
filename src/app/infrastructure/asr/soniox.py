    ...
    def _convert_to_mono_wav(self, audio_path: Path) -> tuple[Path, bool]:
        try:
            ...
            if ext in _FALLBACK_UPLOAD_EXT:
                logger.warning(
                    "ffmpeg unavailable — uploading original %s to Soniox without WAV conversion",
                    ext,
                )
                return audio_path, False
            raise RuntimeError(
                "ffmpeg required in PATH to convert this format, or use .wav. pydub error: %s" % (e,)
            ) from e
    ...
    def _file_duration_sec(self, audio_path: Path) -> float:
        ...
        try:
            ...
            if d is not None and d > 0:
                return d
            raise RuntimeError(
                "Could not read audio duration: install ffmpeg in PATH or pip install mutagen (for m4a/mp3 without ffmpeg)."
            ) from e
    ...
    def transcribe_file(self, audio_path: Path, *, language: str | None = None) -> tuple[str, list[dict], float | None]:
        ...
        if duration_sec <= self.max_duration_sec:
            ...
            try:
                ...
                raise RuntimeError(
                    "Long audio requires ffmpeg in PATH for pydub chunking."
                ) from e
