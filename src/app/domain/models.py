...
class AnalysisResult(BaseModel):
    ...
    transcript_word_segments: list[TranscriptSegment] = Field(
        ...
        description="ASR word-level tokens with speaker labels (finer seek than speaker segments alone).",
    )
...
class PublishSessionBody(BaseModel):
    ...
    public_org: str | None = Field(
        None,
        max_length=200,
        description="Organization label shown on the public page.",
    )
...
class PublicObservationBody(BaseModel):
    ...
    commitment_index: int | None = Field(
        None,
        ge=0,
        description="Commitment row index; omit for whole session.",
    )
    note: str | None = Field(None, max_length=2000)
    photo_url: str | None = Field(None, max_length=2048)
    website: str = Field(default="", description="Honeypot (must stay empty).")
