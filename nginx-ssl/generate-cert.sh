...
# Генерация самоподписанного сертификата с SAN (IP + localhost) для HTTPS по LAN.
# Использование: ./nginx-ssl/generate-cert.sh 192.168.1.42
# Затем: docker compose up -d (или перезапуск https-proxy).
